    ...
    for ii in range(0, a_data.size):
        ...
        for trial in range(0, a_trial.size):
            ...
            data_return[NO_valid_trial, :, :] = np.transpose(
                a_X[int(a_trial[trial]):(int(a_trial[trial]) + window_Length), :3])
...
def data_get(subject):
    x_train = np.loadtxt(
        "D:/pythonProject(pytorch)/EEG-Classification-with-CNN-master/Datos/MI-EEG-A0" + str(subject + 1) + "E.csv",
        delimiter=",")

    x_test = np.loadtxt(
        "D:/pythonProject(pytorch)/EEG-Classification-with-CNN-master/Datos/MI-EEG-A0" + str(subject + 1) + "T.csv",
        delimiter=",")

    y_train = np.loadtxt(
        "D:/pythonProject(pytorch)/EEG-Classification-with-CNN-master/Datos/EtiquetasA0" + str(subject + 1) + "E.csv",
        delimiter=",")

    y_test = np.loadtxt(
        "D:/pythonProject(pytorch)/EEG-Classification-with-CNN-master/Datos/EtiquetasA0" + str(subject + 1) + "T.csv",
        delimiter=",")
    x_train = x_train.reshape(288, 1, 22, 1000)
    x_test = x_test.reshape(288, 1, 22, 1000)
    y_train_onehot = (y_train - 1).astype(int)
    y_train_onehot = to_categorical(y_train_onehot)
    y_test_onehot = (y_test - 1).astype(int)
    y_test_onehot = to_categorical(y_test_onehot)
